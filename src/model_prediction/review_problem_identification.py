...
def run_summary_model(some_review: str) -> str:
    classification_prompt = f"Classsify the following review as either Product Quality Problem, Shipping Problem, Disliked Product or High Price: {some_review}"
    summarization_prompt = f"Summarize the issue in the following product review: {some_review}"
    ...
    api_key = '9u2IIvMhuvfepXqrEXq74SuYeOGykbay'
    if not api_key:
        raise Exception("A key should be provided to invoke the endpoint")

    # The azureml-model-deployment header will force the request to go to a specific deployment.
    # Remove this header to have the request observe the endpoint traffic rules
    headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key), 'azureml-model-deployment': 'google-flan-t5-large-13' }

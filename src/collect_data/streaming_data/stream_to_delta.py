...
df = spark.read.json(all_json_files)\
                ...
                .withColumn("unixReviewTime", func.split(func.split(func.col('source_file'), 'new_event_')\
                                                         .getItem(1),
                                                         '.json'
                                                         )\
                                                  .getItem(0).cast('int')
...
max_time = spark.sql('SELECT MAX(unixReviewTime) AS MAX_ID FROM reviews').toPandas()['unixReviewTime'][0]
